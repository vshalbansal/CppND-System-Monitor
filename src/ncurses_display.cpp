...
void NCursesDisplay::DisplaySystem(System& system, WINDOW* window) {
  ...
  std::vector<Processor> cpus = system.Cpu();
  // CPU utilisation of all system cores
  int numCpus = int(cpus.size());
  ...
  for(int i = 0;i<numCpus;i++) {
    ...
    if(i==0)
      ...
      mvwprintw(window, ++row, 2, "CPU%d:",i-1);
    
    wattron(window, COLOR_PAIR(1));
    mvwprintw(window, row, 10, " ");
    wprintw(window, "%s",ProgressBar(cpus[i].Utilization()).c_str());
...
void NCursesDisplay::DisplayProcesses(std::vector<Process>& processes,
  ...
  for (int i = 0; i < num_processes; ++i) {
    ...
    mvwprintw(window, row, user_column, "%s",processes[i].User().substr(0,9).c_str());
    float cpu = processes[i].CpuUtilization() * 100;
    mvwprintw(window, row, cpu_column, "%s",to_string(cpu).substr(0, 4).c_str());
    mvwprintw(window, row, ram_column, "%s", to_string(processes[i].Ram()).c_str());
...
void NCursesDisplay::Display(System& system, int n) {
  ...
  int numRowsForCpu = int(system.Cpu().size());
  WINDOW* system_window = newwin(9+numRowsForCpu, x_max - 1, 0, 0);
